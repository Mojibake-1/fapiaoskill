...
When the task depends on the `AMZ备货计划及出货安排表` planning sheet, also require a clear user-specified work scope from `备货详情`, such as exact rows/range, FBA shipment ID, or exact SKU/product row set. If the selection is missing or could map to multiple rows/blocks, ask which rows to use before workbook generation.

For `发票产品详情`, the pinned Tencent Docs sheet in the skill entrypoint counts as the current source unless the user explicitly provides a different current source. For all other sources, only treat a source as current when the user explicitly provides or confirms it for this task. Do not search for or reuse local files by generic names such as `发票产品详情.xlsx`, prior `.analysis` outputs, generated corrections JSON, or unrelated completed invoices. If a plausible local product-detail workbook exists but it was not generated by the Tencent Docs helper metadata for the current online version and was not named by the user, ask for confirmation before using it.
...
- Before writing row corrections, confirm every non-export product value can be traced to the pinned Tencent Docs product-detail source or to a user-provided current product-detail source. If the trace is missing, leave the field unresolved and ask for Tencent Docs access repair or a current replacement source.
